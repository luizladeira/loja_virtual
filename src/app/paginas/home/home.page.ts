  ...
  constructor(public http: Http, public serviceUrl: UrlService, public nav: NavController, private route: Router, public serviceUser: ServiceUserService, public menuService: MenuService, public dataService: DataService) {
   ...
   validarMenu(){
      ...
      //fecha as sessões
              ...
              if(this.serviceUser.getUserNivel() == "1") {
                    this.menuService.perfilMenu.push(this.menuService.menu[0]); //cadastro_produto
                    this.menuService.perfilMenu.push(this.menuService.menu[1]);
                    this.menuService.perfilMenu.push(this.menuService.menu[2]); //cadastro_empresa
                   // this.menuService.perfilMenu.push(this.menuService.menu[3]); //cadastro_produto
                    this.menuService.perfilMenu.push(this.menuService.menu[4]); //adicionar_foto
                    this.menuService.perfilMenu.push(this.menuService.menu[5]);
                   // this.menuService.perfilMenu.push(this.menuService.menu[6]);
              }

              
              if(this.serviceUser.getUserNivel() == "2" ){
                    this.menuService.perfilMenu.push(this.menuService.menu[0]); //cadastro_produto
                    this.menuService.perfilMenu.push(this.menuService.menu[1]);
   ...
   editarProduto(produto){
    ...
    this.nav.navigateForward('/editar-produtos');
      //this.route.navigate(['cadastro-produtos'],this.detalhe); //envio todos os itens envio atraves da classe route e consigo enviar os dados para outra tela
   ...
   listarProduto(produto){
    this.dataService.storange = {
      ...
      this.nav.navigateForward('list-produtos');
   }
